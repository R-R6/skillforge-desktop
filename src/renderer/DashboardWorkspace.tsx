...
export default function DashboardWorkspace({ onOpenLibrary, onOpenProjects, onOpenPresets }: DashboardWorkspaceProps) {
  ...
  return <section className="dashboard-workspace">
    <div className="dashboard-intro">
      <div><span className="eyebrow">WORKSPACE OVERVIEW</span><h2>今天从管理你的 AI 能力开始</h2><p>SkillForge 负责保存 Skill，再把选中的能力部署到具体项目和编码 Agent。</p></div>
    ...
    <div className="dashboard-metrics">
      ...
      <MetricCard icon={<FolderKanban size={17} />} label="已管理项目" value={loading ? "—" : projects.length} detail={`${boundProjects} 个已有绑定`} />
      <MetricCard icon={<Tags size={17} />} label="Preset 预设" value={loading ? "—" : presets.length} detail="可一键应用到项目" />
      <MetricCard icon={<Bot size={17} />} label="支持 Agent" value="4" detail="Codex · Cursor · Claude · Hermes" />
    </div>

    <div className="dashboard-columns">
      <div className="dashboard-card">
        <div className="dashboard-card-heading"><div><span className="eyebrow">RECENT SKILLS</span><h3>最近可用的 Skill</h3></div><button className="text-button" onClick={onOpenLibrary}>查看全部 <ArrowRight size={14} /></button></div>
      ...
      <div className="dashboard-card">
        <div className="dashboard-card-heading"><div><span className="eyebrow">QUICK ACTIONS</span><h3>快速开始</h3></div><CheckCircle2 size={18} className="dashboard-check" /></div>
        <div className="quick-action-list"><button className="quick-action" onClick={onOpenProjects}><span><FolderKanban size={16} /><strong>导入一个项目</strong><small>选择目标 Agent 并部署 Skill</small></span><ArrowRight size={15} /></button><button className="quick-action" onClick={onOpenPresets}><span><Tags size={16} /><strong>创建 Preset</strong><small>保存一组常用的 Skill 组合</small></span><Plus size={15} /></button><button className="quick-action" onClick={onOpenLibrary}><span><Library size={16} /><strong>管理 Skill 库</strong><small>搜索、打标签、启用或禁用</small></span><ArrowRight size={15} /></button></div>
      </div>
    </div>
  </section>;
}
