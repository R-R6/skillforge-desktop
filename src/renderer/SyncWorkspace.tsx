...
export default function SyncWorkspace({ onOpenProjects }: SyncWorkspaceProps) {
  ...
  return <section className="sync-workspace">
    <div className="section-toolbar project-toolbar"><div><h2>部署状态</h2><span>查看项目内 Agent 入口与 Skill 文件分布，扫描不会修改项目内容。</span></div><div className="project-actions"><button className="ghost-button" onClick={() => refresh()}><RefreshCw size={15} /> 刷新</button><button className="primary-button" onClick={onOpenProjects}><FolderKanban size={16} /> 前往项目管理</button></div></div>
